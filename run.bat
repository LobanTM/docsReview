...
:build_share
    ...
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f docsreview-share
	call %MVN_EXEC% clean package -pl docsreview-share,docsreview-share-docker
EXIT /B 0
:build_acs
    docker-compose -f "%COMPOSE_FILE_PATH%" kill docsreview-acs
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f docsreview-acs
	call %MVN_EXEC% clean package -pl docsreview-integration-tests,docsreview-platform,docsreview-platform-docker
EXIT /B 0
:tail
...
EXIT /B 0
:prepare-test
    call %MVN_EXEC% verify -DskipTests=true -pl docsreview-platform,docsreview-integration-tests,docsreview-platform-docker
EXIT /B 0
:test
    call %MVN_EXEC% verify -pl docsreview-platform,docsreview-integration-tests
EXIT /B 0
:purge
